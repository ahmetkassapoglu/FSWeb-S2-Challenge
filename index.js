...
let cumle = cumleler.map(kelime => kelime.join(separator))
return cumle
}
...
let tekler= cumleler.filter(cumle => cumleler.indexOf(cumle) %2 !== 0)
let birucbes = []
for (let i=0; i<5; i++){
	birucbes.push(tekler[i])
}
let cumle = cumlelereDonustur(birucbes," ")
let son = cumle.join("")
let paragraf= cumleKur(son)
return paragraf
...
Tavşanla kirpi sebzeleri ele geçirmek için bir plan kurdular.
Tavşan diziye önden saldıracak, kirpi ise arkalarından dolaşacak.
Varsayalım ki arkadaşların planları başarılı oldu. Tavşanı dizinin ilk elemanına 🐇,
...
/* 	GÖREV 4:
		Yeni kurulmuş bir mesajlaşma startup firması atılan mesajları emojilerle zenginleştirmek istiyor. Bunun için emojiler adında bir nesne tanımlamışlar. Kullanıcının gönderdiği mesaj stringi içinde emojiler nesnesinin anahtarı(index) bulunuyorsa,
		 ...
		 ÖRNEK: Mesaj içinde ":)" sembolü bulunursa mesajı alan kişi bu sembolü "🙂" olarak görecek.
		 Burdan yola çıkarak emojileriDonustur fonksiyonuna aşağıdakileri uygulayın.
